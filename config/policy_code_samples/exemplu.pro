/*
[f1] passwd( mirela, alerim).					//=>	rule( f1, passwd( mirela, alerim), []).	
[f2] user1[name:mirela,passwd:alerim].			//=>	rule( f2,complex_term(user1,name,mirela),[]).
												//		rule( f2,complex_term(user1,passwd,alerim),[]).
[f3] valid(user1[name:mirela,passwd:alerim]).	//=>	rule( f3,valid(user1),[]).
...
[r3] U[name:N,age:A]:-getUsers(U,N,A),A>21.		
		//=>	rule( r3, complex_term(U,name,N),[getUsers(U,N,A),A>21]).
		//		rule( r3, complex_term(U,age,A),[getUsers(U,N,A),A>21]).

[r4] valid(U[name:N,age:A]):-getUsers(U,N,A),A>21.
		//		=>	rule( r4, valid(U), [getUsers(U,N,A),A>21]).
		//		rule( r4, complex_term(U,name,N),[getUsers(U,N,A),A>21]).
		//		rule( r4, complex_term(U,age,A),[getUsers(U,N,A),A>21]).

[r5] select_age(mirela,A):- U[ name:mirela, age:A].
		//=>	rule( r5, select_age(mirela,A), [complex_term(U,name,mirela),complex_term(U,age,A)].

[r6] check(mirela,23):- valid(U[ name:mirela, age:23]).
		//=>	rule( r6, check(mirela,23), [valid(U),complex_term(U,name,mirela),complex_term(U,age,23)].


[r2].sensitivity:public.					
		//=> 	metarule(id,sensitivity(r2,public),[]).
passwd(_,_).sensitivity:private.		
		//=> 	metarule(pred,sensitivity(passwd(_,_),private),[]).
user1[name:N,age:A].sensitivity:private.		
		//=>	metarule(pred,sensitivity(user1,private),[complex_term(user1,name,N),complex_term(user1,age,A)]
*/
