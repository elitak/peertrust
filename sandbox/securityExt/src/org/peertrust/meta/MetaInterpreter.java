...
/**
 * $Id: MetaInterpreter.java,v 1.1 2004/12/21 11:09:34 seb0815 Exp $
 * @author olmedilla
 * @date 05-Dec-2003
...
public class MetaInterpreter implements Configurable, Runnable
{
	...
	void processQueue()
	{
				  ...
				  ( selectedTree.getStatus() == Tree.ANSWERED) )
		{
			...
			Answer answer = new Answer (selectedTree.getGoal(), selectedTree.getProofRuleVector(), status, selectedTree.getReqQueryId(), localPeer) ;
			
			log.debug("Sending answer " + answer.getGoal() + " to " + selectedTree.getRequester().getAlias() + " with proof " + answer.getProofRuleVector()) ;
			sendMessage(answer, selectedTree.getRequester()) ;
			return ;
		}
		//If an authenticatesTo-predicate has to be evaluated
		if(AuthenticatesTo.authenticatesToPredicate(selectedTree,this,entities,
			localPeer,config,queue))
			return;
		//If query says that local peer has to authenticate himself to satisfy the
		//authenticatesTo-predicate
		if(AuthenticatesTo.authenticatesPredicate(selectedTree,config,localPeer,this))
			return;
			...
			for (int i = 0 ; i < results.length ; i++)
			{
				...
				if (delegator == null)
				{
					...
					if (results[i].getSubgoals().equals("[]"))
					{
						...
						answeredTree.appendProofRuleVector(credStore,
							config,results[i].getProof());
					...
					else // otherwise, we add the query to the queue
					{
						...
						Tree newTree = new Tree(results[i].getGoal(), results[i].getSubgoals(), selectedTree.getProofRuleVector(), Tree.READY, selectedTree.getRequester(), selectedTree.getReqQueryId()) ;
						newTree.appendProofRuleVector(credStore,config,
							results[i].getProof()) ;
						queue.add(newTree) ;
					}
				}
				else // we delegate
				{
					...
					Tree delegatedTree = new Tree (results[i].getGoal(), results[i].getSubgoals(),
							selectedTree.getProofRuleVector(), Tree.WAITING, selectedTree.getRequester(),
							selectedTree.getReqQueryId(), (Peer) entities.get(delegator), results[i].getGoalExpanded()) ;
					delegatedTree.appendProofRuleVector(credStore,config,
						results[i].getProof()) ;
	...
	public void sendMessage(Message message, Peer destination)
	{
