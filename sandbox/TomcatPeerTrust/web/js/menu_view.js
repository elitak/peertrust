...
function on_menu_item_mouseover(entity){
	entity.style.backgroundColor="ButtonShadow";
	entity.style.cursor='pointer';
	return;
}

function on_menu_item_mouseout(entity){
	entity.style.backgroundColor="ButtonFace";
	entity.style.cursor='Default';
}

function setStyle(object,propKey,propValue){
	object.style[propKey]=propValue;
	return;
}

function onButtonClick(linkToLoad){
	alert(linkToLoad);
}
...
/////////////////////////////////////////////////////////////////////
		...
		var intervalID;
		var intervalCount=0;
		var progressMaxTime=5;
		
		function createProgressBar(containerID, totalTimeInSec){
			progressMaxTime=totalTimeInSec;
			alert("dada");
			try{
				var container=document.getElementById(containerID);
							
				var pBar= document.createElement("DIV");
				pBar.id="_progressBar";
				pBar.style.width="100%";
				pBar.style.height="100%";
				pBar.style.backgroundColor="white";
				pBar.style.borderStyle="inset";
				pBar.style.borderColor="silver";
			
			//addProgressBarTD(pBar);
			
				var resizableBar= document.createElement("DIV");
				resizableBar.id="_resizableBar";
				resizableBar.style.width="10%";
				resizableBar.style.height="100%";
				resizableBar.style.backgroundColor="ButtonFace";
				//resizableBar.bgColor="ButtonFace";
				
				//resizableBar.style.borderStyle="inset";
				//resizableBar.style.borderColor="silver";
				
				//bild it and append to container
				pBar.appendChild(resizableBar);
				container.appendChild(pBar);
				//alert(container.nodeName);
				intervalID=setInterval("progress()",100);
			}catch(ex){
		...
		function progress(){
			var resizableBar=document.getElementById("_resizableBar");
			var sizeStr= String(resizableBar.style.width);
			
			//alert(sizeStr);
			
			size=Number(sizeStr.substring(0,sizeStr.length-1));
			size=size+10;
			//alert("sizeStr:"+sizeStr+" size:"+size);
			if(size>100){
				size=0;
				intervalCount=intervalCount+1;
				if(intervalCount>=progressMaxTime){
					clearInterval(intervalID);
					return;
				}
			}
			resizableBar.style.width=size+"%";
			return;
		}
		
		function stopAndHideProgressBar(containerID){		
			var container= document.getElementById(containerID);
			clearInterval(intervalID);
