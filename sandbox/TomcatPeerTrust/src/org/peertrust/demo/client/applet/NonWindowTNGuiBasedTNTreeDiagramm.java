...
public class NonWindowTNGuiBasedTNTreeDiagramm {
     ...
     */
    public NonWindowTNGuiBasedTNTreeDiagramm(Graphics graphics) {
...
//	                + targetPort;
	...
	* @param reQueryId The reqQueryId.
    ...
    private String createNode(Object object, int x, int y, long reqQueryId,
        ...
        Rectangle nodeBounds = new Rectangle(x, y, graphics.getNodeBoundsX(),
                graphics.getNodeBoundsY());
	...
	* @param reQueryId The reqQueryId.
    ...
    public void refreshGraph() {
        ...
        graph.getGraphLayoutCache().setVisible(
                new Object[] { graphics.getRoot() }, false);
    ...
    private void positionNode(TNNode node, int x, int y) {
        ...
        positionNode(node, x, y, graphics.getNodeBoundsX(), graphics
                .getNodeBoundsY());
	...
	//Calculate the vertical distance between two tree depth levels, so that all is visible on the screen
        int paneHeight=((JPanelGuiBasedTNGraphics)graphics).getJPanelTNGui().getPaneHeight();
	...
	//Iterate through all depth levels of the tree
		...
		//levels, so that all is visible on the screen
            x_delta = (((JPanelGuiBasedTNGraphics)graphics).getJPanelTNGui().getPaneWidth() - graphics.getNRadius())
                    / anzahl;
		...
		//Position every node in the current tree depth level
            for (int j = stufen[i]; j < stufen[i] + anzahl; j++) {
                ...
                positionNode(node, (j - stufen[i] + 1)
                        * ((JPanelGuiBasedTNGraphics)graphics).getJPanelTNGui().getPaneWidth() /

                        (anzahl + 1) - graphics.getNRadius() / 2, (node
                        .getStufe() - 2)
                        * y_delta);
