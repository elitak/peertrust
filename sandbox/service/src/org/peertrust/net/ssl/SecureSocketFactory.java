...
/**
 * $Id: SecureSocketFactory.java,v 1.1 2004/08/07 12:51:56 magik Exp $
 * 
 * @author olmedilla
...
public class SecureSocketFactory implements AbstractFactory {
    ...
    public NetClient createNetClient(Configurator config) {
        ...
        String keystoreFile = config.getValue(Vocabulary.BASE_FOLDER_TAG)
                + config.getValue(Vocabulary.KEYSTORE_FILE_TAG);
        ...
        log.debug("keystoreFile: " + keystoreFile + " - " + "keyPassword: "
                + keyPassword + " - " + "storePassword: " + storePassword);
    ...
    public NetServer createNetServer(Configurator config) {
        ...
        int port = Integer
                .parseInt(config.getValue(Vocabulary.SERVER_PORT_TAG));
        String keystoreFile = config.getValue(Vocabulary.BASE_FOLDER_TAG)
                + config.getValue(Vocabulary.KEYSTORE_FILE_TAG);
        String keyPassword = config.getValue(Vocabulary.KEY_PASSWORD_TAG);
        String storePassword = config.getValue(Vocabulary.STORE_PASSWORD_TAG);
        log.debug("port: " + port + " - keystoreFile: " + keystoreFile
                + " - keyPassword: " + keyPassword + " - storePassword: "
                + storePassword);

        return new SecureServerSocket(port, keystoreFile, keyPassword,
                storePassword);
    }
}