...
//javax.swing.JOptionPane.showMessageDialog(null,"Query: "+strTrace[strTrace.length-1]+" "+strTrace.length);
		if(strTrace.length==1) {			log.debug (query.getTrace().printTrace() + "length 1") ;
			TNNode source=graphics.getRoot();
 			TNNode target = (TNNode)getElement(createNode(sourceAlias,reqQueryId,
				sourceAddress,sourceAlias,sourcePort));
 			TNEdge edge = (TNEdge)getElement(connectNodes(source,target,"","",
				-1,true,false,-1,""));
 			graphPath.add(edge);
 			graphPath.add(target);
 			TNNode target2 = (TNNode)getElement(createNode(targetAlias,reqQueryId,
				targetAddress,targetAlias,targetPort));
 			edge = (TNEdge)getElement(connectNodes(target,target2,goal+"?",
	...
	public void addAnswer(Answer answer) {
		...
		if(strTrace.length==1)
			return;
//javax.swing.JOptionPane.showMessageDialog(null,"Answer: "+strTrace[strTrace.length-1]+" "+strTrace.length);
		long id=reqQueryId;
		TNNode node=null;
		for(int i=0;i<strTrace.length-1;i++) {
//			if(strTrace[i].startsWith("<"))
//				strTrace[i]=strTrace[i].substring(1,strTrace[i].length()-1);
//javax.swing.JOptionPane.showMessageDialog(null,strTrace[i]);
			if((i==0)&&(listEdges.size()==0)) {
			...
			for(int j=0;j<list_edges.size();j++) {
				if(((TNEdge)list_edges.get(j)).getLabel().
					equals(strTrace[i])) {
//javax.swing.JOptionPane.showMessageDialog(null,"yup");
					node=((TNEdge)list_edges.get(j)).
						getTargetNode();
					break;
				}
