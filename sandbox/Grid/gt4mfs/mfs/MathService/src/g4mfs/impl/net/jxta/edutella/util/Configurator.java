...
public class Configurator {
	...
	public void init(Properties props, String[] clArgs) {
		...
		addOption(
			new Option('h', "help", "print this help", false, "false", true));
	}
	/**
	...
	private void initConfig() {
		if (!initialized) {
			...
			setProperties();
			// parse command line args and overwrite the respective properties
			setArgs(args);
	...
	public void addOption(Option opt) {
		if (options.containsKey(opt.longName)) {
			throw new IllegalArgumentException(
				"option name '" + opt.longName + "' used twice");
		}
		options.put(opt.longName, opt);
		if (opt.hasShortName()) {
			if (options.containsKey(opt.shortName)) {
				throw new IllegalArgumentException(
					"option name '" + opt.shortName + "' used twice");
			}
...
//	private void configureAll() {
	public void configureAll() {
		// configure all registered objects
	...
	private void retrieveOptionValues() {
		// get values of registered objects back
		for (Iterator iter = configurables.iterator(); iter.hasNext();) {
			Configurable conf = (Configurable) iter.next();
	...
	public void finishConfig() {
		...
		if (getMissingOptions().size() > 0) {
			ConfigDialog cd =
				new ConfigDialog(
					...
					"Please supply configuration information",
					this);
			cd.showDialog();
		...
		// required values can be set anywhere (command line,
		// properties file, config dialog), but they must be
		// available
	...
	public Collection getMissingOptions() {
		...
		for (Iterator i = options.values().iterator(); i.hasNext();) {
			...
			if (!optList.contains(opt)
				&& opt.isRequired()
				&& (opt.getValue() == null || opt.getValue() == "")) {
				optList.add(opt);
			}
		}
		return optList;
	...
	public void setArgs(String[] args) {
		//index of arguments which are not of the form '-<name> <val>'
	...
	public String getValue(String optName) {
		...
		if (opt.required) {
			throw new RuntimeException(
				"no value set for required argument '" + optName + "'");
		} else {
				...
				if (setter != null) {
					...
					try {
						if (isValueAvailable(propertyName)) {
							...
							isValueAvailable(propertyName.toLowerCase())) {
							option =
								(Option) options.get(
						...
						getLogger().error(
							"access rights prohibit to set attribute '"
								+ attribute
								+ "'",
							e);
	...
	public void initObject(Configurable object) {
		...
		try {
			...
			getLogger().error(
				"access rights prohibit invoking init(EduConfig)",
				e);
		} catch (InvocationTargetException e) {
			getLogger().error(
				"error invoking init(EduConfig) on object " + object,
				e);
		}
		if (!initInvoked) {
			try {
				Method initMethod =
					object.getClass().getMethod("init", new Class[] {
				});
				...
				getLogger().error(
					"error invoking init() on object " + object,
					e);
			}
		}
	}
			...
			for (int i = 0; i < propertyDescriptors.length; i++) {
				...
				if (getter != null) {
					...
					try {
						...
						getLogger().error(
							"access rights prohibit to get attribute '"
								+ attribute,
							e);
					} catch (InvocationTargetException e) {
