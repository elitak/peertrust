...
/**
 * The interface <tt>Mapper</tt> provides conversion utilities between the
 * Java (resp. Prolog) part and the Prolog (resp. Java) part of the system. The
 * <i>Protune</i> system deals with a number of entities (e.g. notifications,
 * checks, policies ...) which are represented in different ways in the Java
 * (resp. Prolog) subsystem. The methods of the interface <tt>Mapper</tt>
 * convert these entities between the one and the other representation.<br />
 * <b>NOTE:</b> The interface <tt>Mapper</tt> should be aware of each entity
 * available in the system (which requires to be represented both in the Java
 * and Prolog subsystems), therefore whenever a new entity of this kind is
 * added, the interface <tt>Mapper</tt> (all classes implementing it) should
 * be modified. On the other hand it provides many advantages.
 * <ul>
 * <li>It would have been possible to define for each entity available in the
 * system
 * <ul>
 * <li>a constructor <tt>&lt;entityName&gt;(String)</tt> creating a Java
 * object (a Java representation of the entity) starting from its Prolog
 * representation.</li>
 * <li>a method <tt>toString()</tt> returning a string representing the
 * entity (a Prolog representation of the entity) starting from its Java
 * representation.</li>
 * </ul>
 * In this case <em>each class</em> defining the entity should have been
 * modified upon changes in the Prolog representation. </li>
 * <li>With our choice a change in the Prolog representation simply requires a
 * new implementation of the interface <tt>Mapper</tt>.</li>
 * </ul>
 * The interface <tt>Mapper</tt> provides two kinds of methods
 * <ul>
 * <li>methods converting entities from the Java representation to the Prolog
 * one</li>
 * <li>methods converting entities from the Prolog representation to the Java
 * one</li>
 * </ul>
 * Since the way of querying the Prolog engine depends on the representation of
 * the entities stored in the engine's state, a third kind of methods is
 * required
 * <ul>
 * <li>query methods, returning the Prolog goal which should be proved by the
 * Prolog engine when needed. Have a look to the class
 * {@link org.protune.core.ProtuneFilterEngine} in order to understand how they
 * can be used.</li>
 * </ul>
 * Tab. 1 lists the various kinds of methods. <table border="1" cellspacing="0">
 * <thead>
 * <tr>
 ...
 * </td>
 * </tr>
 * </tbody> </table> <b>Tab. 1</b> - <tt>Mapper</tt> methods overview.
 * 
 * @author jldecoi
 */
public interface Mapper {
	...
	/**
	 * Translates a Java array into a Prolog representation of a list. The
	 * elements contained in the array are supposed to be Java object the Mapper
	 * is aware of (i.e. {@link org.protune.api.Notification},
	 * {@link org.protune.api.Check}, {@link org.protune.api.Policy} ...),
	 * therefore for each object an invocation to the <tt>accept(Mapper)</tt>
	...
	public ActionWellPerformed parseActionWellPerformed(String s)
			throws ParseException;

	public ActionWrongPerformed parseActionWrongPerformed(String s)
			throws ParseException;

	public NotificationReliable parseNotificationReliable(String s)
			throws ParseException;

	public NotificationUnreliable parseNotificationUnreliable(String s)
			throws ParseException;

	public NegotiationElement parseNegotiationElement(String s)
			throws ParseException;
	...
	/**
	 * Translates a Prolog representation of a list into a Java array. The
	 * elements contained in the list are supposed to be (Prolog representations
	 * of) entities the Mapper is aware of (i.e. notifications, checks, policies
	 * ...), therefore for each element an invocation to the suitable
	...
	/**
	 * Something like <blockquote><tt>return "add_to_session( 0, received( " + negotiationElement +
	 * " ) ).";</tt></blockquote>
	 * 
	...
	/**
	 * Something like <blockquote><tt>return "start_session( 0, allow( " + action + " ) ).";</tt></blockquote>
	 * 
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return "filter_policy1( 0, ActionList ).";</tt></blockquote>
	 * 
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return list.matches("\\[\\]");</tt></blockquote>
	 * 
	 * @param list
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return "filter_policy2( 0, " + notificationList + ", ActionList
	 * ).";</tt></blockquote>
	 * 
	...
	/**
	 * Something like <blockquote><tt>return "filter_policy3(0).";</tt></blockquote>
	 * 
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return "session(0, X), X = rule(_, _, _).";</tt></blockquote>
	 * 
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return "session(0, X), X = metarule(_, _, _).";</tt></blockquote>
	 * 
	 * @return
	 */
	...
	/**
	 * Something like <blockquote><tt>return "session( 0, state( received( session( 0, state( received( " +
